...
type_abbrev["agents agent nonagent"]                                  = tc.ABBREV_AGENT_NON_AGENT
type_abbrev["agents imitation"]                                       = tc.ABBREV_SOCIAL_IMITATION
type_abbrev["agents approach"]                                        = tc.ABBREV_SOCIAL_APPROACH
...
def get_scene_name_including_answer(scene_id, entry_fields):
    ...
    gt_expectedness_numerical = entry_fields[val_index["EVALUATION SCORE NUMERICAL GROUND TRUTH"]] #Ridha found this column to be more accurate, TA2 confirmed 
    if gt_expectedness_numerical == "1":
        # more expected of the pair
        expectedness = "mexpected"
    else:
        # less expected of the pair
        expectedness = "lexpected"
    return (
        type
        + "_"
        + scene_id
